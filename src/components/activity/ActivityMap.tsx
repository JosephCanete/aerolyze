...
export function ActivityMap({ activity }: ActivityMapProps) {
  ...
  const decodePolyline = async (str: string): Promise<[number, number][]> => {
    if (typeof window !== "undefined") {
      const polylineLib = await import("polyline");
      return polylineLib.decode(str) as [number, number][];
    }
    return [];
  };
  ...
  useEffect(() => {
    if (activity.map?.summary_polyline) {
      decodePolyline(activity.map.summary_polyline).then(setRouteCoordinates);
    }
    ...
    return (
      <div className="bg-gray-50 rounded-lg p-8 text-center border-2 border-dashed border-gray-200">
        ...
        <p className="text-sm text-gray-500 max-w-sm mx-auto">
          This activity doesn&apos;t have GPS route information. Route maps are
          available for activities recorded with GPS-enabled devices.
        </p>
      </div>
    );
  }
  ...
  const createCustomIcon = async (color: string, symbol: string) => {
    if (typeof window !== "undefined") {
  ...
  const [finishIcon, setFinishIcon] = useState<any>(null);

  // Create icons on component mount
  useEffect(() => {
    createCustomIcon("#22c55e", "S").then(setStartIcon);
    createCustomIcon("#ef4444", "F").then(setFinishIcon);
  }, []);
